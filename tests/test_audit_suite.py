...
# Guarantee QApplication singleton exists for widget instantiation
...
from gui.widgets.thumbnail_loader import ThumbnailCache
...
class TestThreadSafetyAndMemory(unittest.TestCase):
    ...
    def test_thumbnail_cache_thread_safety(self) -> None:
        errors: list[str] = []
    ...
    def test_media_card_lifecycle_and_teardown(self) -> None:
        item_data = {
            "id": "987654",
...
class TestNetworkResilienceAndFallbacks(unittest.TestCase):
    ...
    def test_http_429_rate_limit_resilience(self) -> None:
        worker = InspectWorker(
            targets=["https://www.instagram.com/test_creator/reels/"]
        )
    ...
    def test_single_post_embed_json_and_img_index_extraction(self) -> None:
        ...
        self.assertEqual(card1["thumbnail_url"], "https://cdn.example.com/slide1.jpg")
...
class TestURLParsingAndNormalization(unittest.TestCase):
    ...
    def test_tracking_params_and_punctuation_stripping(self) -> None:
        raw_urls = [
            (
...
class TestUIResponsivenessAndBatchRendering(unittest.TestCase):
    ...
    def test_batch_card_addition_and_selection(self) -> None:
        win = MainWindow()
        for i in range(50):
            item_data = {
                "id": str(1000 + i),
        ...
        self.assertEqual(len(win.cards), 50)
        # Verify formatting matches: 'Media Queue (50/50 Selected)'
        self.assertIn("50/50", win.lbl_selected_count.text())
