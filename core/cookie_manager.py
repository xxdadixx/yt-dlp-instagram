...
class CookieManager:
    ...
    def __init__(self, cookie_file: Optional[str] = None):
        ...
        self.cookie_file_path: str = os.path.abspath(
            cookie_file or self.DEFAULT_COOKIE_FILE
        )
    ...
    def save_to_netscape_file(self, file_path: Optional[str] = None) -> bool:
        target_path = file_path or self.cookie_file_path
        if not self.cookies or not target_path:
            return False
        ...
        try:
            ...
            for name, val in self.cookies.items():
                lines.append(f".instagram.com\tTRUE\t/\tTRUE\t{expiry}\t{name}\t{val}")
            ...
            logger.error(f"Failed to save Netscape cookie file: {e}")
            return False
